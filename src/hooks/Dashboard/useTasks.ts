...
const useTasks = () => {
  ...
  const groupTaskByStatus = (taskArray: Task[]) => {
    return taskArray.reduce((orderTasks, currentItem) => {
      ((orderTasks)[currentItem.status] = orderTasks[currentItem.status] || []).push(currentItem)
      return orderTasks
    }, {})
  }
  ...
  const taskClassification = () => {
    ...
    const groupedTask = Object.keys(tasksSorted).map((key: string) => {
